...
class SkillsAndI18nTests(unittest.TestCase):
    ...
    def test_core_skill_guide_mentions_narrow_scope(self) -> None:
        ...
        self.assertIn("planning spans multiple phases", text)
        self.assertIn("Do not leave", text)
        self.assertIn("only in chat", text)
        ...
        self.assertIn("long-term direction", full_text)
        self.assertIn("known later phases", full_text)
        self.assertIn("next intended phase only in chat history", full_text)
