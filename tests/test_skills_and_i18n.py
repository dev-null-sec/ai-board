...
class SkillsAndI18nTests(unittest.TestCase):
    ...
    def test_readme_guides_human_agent_collaboration_with_prompts(self) -> None:
        ...
        self.assertIn("vibe coding 里一个没人管的盲区", chinese)
        self.assertIn("AI 不能自行推断项目方向", chinese)
        self.assertIn("每个代码变更必须有归属", chinese)
        self.assertIn("多 agent 不能互踩", chinese)
        self.assertIn('验收不是"看起来对了"', chinese)
        self.assertIn("AI 的每次改动都应该挂在一个声明了 scope 的任务下面", chinese)
        self.assertIn("CLI 不会在运行时拦截文件写入", chinese)
        self.assertIn("### 人怎么用", chinese)
        self.assertIn("### AI 怎么用（供 Claude/Codex 等 agent 参考）", chinese)
        self.assertIn("用 ai-board 接手这个项目", chinese)
        self.assertIn("空项目不会直接写代码，会先让你确认目标", chinese)
        self.assertIn("把这个需求进 ai-board。如果不属于当前任务就排期", chinese)
        self.assertIn("scope 写什么、验收标准怎么定", chinese)
    ...
    def test_readme_documents_multi_agent_opt_in_default_off(self) -> None:
        ...
        self.assertIn("单 agent 开发时不强制这些检查", chinese)
        self.assertIn("不会天天让你处理 notice 和 scope 冲突", chinese)
        self.assertIn("scope 冲突和被阻塞的候选任务", chinese)
    ...
    def test_readme_documents_git_first_without_silent_init(self) -> None:
        ...
        self.assertIn("它依赖 git", chinese)
        self.assertIn("建议编码前提交", chinese)
        self.assertIn("git_integration=required", chinese)
        self.assertIn("静默 `git init` / `git commit`", chinese)
